...
export default function Map() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      d3.json('/hospital.geojson'),
    ]).then(
      ([world, policeGeojson, fireGeojson, countyGeojson, hospitalGeojson]) => {
        ...
        g.selectAll('.hospital-circle')
          .data(hospitalGeojson.features)
          .enter()
          .append('circle')
          .attr('class', 'hospital-circle')
          .attr('cx', (d) => projection(d.geometry.coordinates)[0])
          .attr('cy', (d) => projection(d.geometry.coordinates)[1])
          .attr('r', 5)
          .attr('fill', 'green')
